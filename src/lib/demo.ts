...
export class DemoApi {
  ...
  /** Демо: статистики источников нет */
  async getVisitsBySource(_days: number): Promise<VisitSourceRow[]> {
    return [];
  }

  async getVisitsCountrySeries(_country: string, _days: number): Promise<VisitCountryDay[]> {
    return [];
  }
