...
const SYSTEM_PROMPT =
  ...
  '8) Группируй по площадке: ОДНА площадка с программой разных танцев/шоу по дням недели — это ОДИН элемент (перечисли программу по дням в description), а не отдельный элемент на каждый танец. Разные площадки — разные элементы. Если на странице нет ни одного конкретного представления — верни {"events": []}.';
