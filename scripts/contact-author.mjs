// ОТКЛЮЧЁН (13.09.2026, решение Дмитрия). Модуль НЕ импортируется ни одним
// скриптом: определение аккаунта по подписи поста («Пост от: Misha | MISH.REC»
// → @mish_rec) отключено — риск ложного сопоставления признан слишком высоким.
// Контакты берём ТОЛЬКО тем, что реально есть в HTML/тексте поста:
//   * ссылки поста (collect-tg.pickLinks → contact_telegram),
//   * extractContacts из текста (scripts/contacts-regex.mjs): WA/тел/почта/инста.
// Файл оставлен как справка (разбор подписи + проверка display-name аккаунта);
