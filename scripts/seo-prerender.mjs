...
const CITY_SEO = {
  ...
  'da-nang': {
    h1: 'Мероприятия и события в Дананге',
    intro:
      'Дананг — быстрорастущий город Вьетнама, где живёт и отдыхает много экспатов и туристов. На карте MyPins — актуальные мероприятия и события Дананга: вечеринки и концерты, спорт и йога, маркеты и встречи для экспатов. Организаторы публикуют события сами, поэтому афиша всегда живая. Фильтры по дате, категории, цене и языку помогут найти занятие на вечер или на выходные.',
    faq: [
      {
        q: 'Какие события и мероприятия проходят в Дананге?',
  ...
  'nha-trang': {
    h1: 'Мероприятия и события в Нячанге',
    intro:
      'Нячанг — главный пляжный курорт Вьетнама и популярное место зимовки туристов и экспатов. На карте MyPins собраны мероприятия Нячанга: концерты и вечеринки, speaking-клубы и встречи, йога, шоу и маркеты. Афиша пополняется организаторами напрямую — вы всегда видите актуальные даты, места и цены. Фильтры по дате, категории и цене помогут выбрать, куда сходить.',
    faq: [
      {
        q: 'Какие события и мероприятия проходят в Нячанге?',
...
function citySeoHtml(seo, evs, categoriesHtml = '') {
  ...
  if (Array.isArray(evs) && evs.length) {
    // «Мероприятия и события на Бали» / «События на Бали» → «Ближайшие события
    // на Бали» (падежная часть h1 переиспользуется). Заголовки городов сейчас
    // начинаются с «Мероприятия и события …» — старый шаблон `^События\s+` их
    // не отрезал и в статике выходило «Ближайшие события Мероприятия и события
    // на Бали» (найдено 17.09.2026 на /bali/, /da-nang/, /nha-trang/).
    const where = seo.h1.replace(/^(?:Мероприятия и события|События)\s+/, '');
...
function categoryH1(cat, path, lang) {
  return lang === 'en'
    ...
    : `${cat.name_ru} ${CAT_CITY_WHERE_RU[path]}: афиша и мероприятия`;
}
